...
var UrlUtil = (function () {
    ...
    return {
        BuildUrl: function (id, urlType) {
            ...
            var useInternal = window.location.protocol == 'https:';
            var RepositoryGlobalSettings;
            if (!useInternal) {
                RepositoryGlobalSettings = {
                    url: 'http://av.epo.pl/RepositoryAccess/',
                    metadata_url: "http://av.epo.pl/RepositoryAccess/meta/",
                    subtitles_url: "http://av.epo.pl/RepositoryAccess/"
                };
            } else {
                RepositoryGlobalSettings = {
                    url: "http://av.epo.pl/RepositoryAccess/",
                    subtitles_url: "//av.epo.pl/reader/utils/video/",
                    metadata_url: "//av.epo.pl/reader/utils/video/meta/"
                };
            }
        ...
        BuildMediaSource: function (mediaType, id, level, profiles, vttSubtitles, track, subtitle) {
            ...
            var metadata = new Metadata(id, mediaType == MediaType.Video);
            profiles = this.UpdateProfilesList(metadata);
...
function createMediaPlayer(container, id, settings, isVideo) {
   ...
   var allowVttSubtitles = true;
    ...
    var defaultVideoSettings =
    {
        ...
        generatehtml: true,
        timeline: false,
        error_distribution: 'http://rt.epo.pl/repo/avplayer/images/distribution.jpg',
        error_absent: 'http://rt.epo.pl/repo/avplayer/images/absent.jpg',
        error_noway: 'http://rt.epo.pl/repo/avplayer/images/noway.jpg',
        aspectRatio: 1.77778,
        Debug: false,
    ...
    this.OnSetNewMetadata = function (id, result, updatedProfiles, vttSubtitles) {
        ...
        setDurtion(result.Duration);

        if (result.Duration != undefined)
            mediaSources.duration = result.Duration;
        ...
        if (isVideo) {
            _this_.FillAudioTracks(result.AltAudio);
            _this_.FillSubtitlesTracks(result.Subtitles);
        }
        _this_.setQualityTracks(result.Profiles);
    ...
    function getMetadataInfo(id, isVideo) {
        ...
        var updatedProfiles = UrlUtil.UpdateProfilesList(metadata);
        var vttSubtitles = UpdateVttTrack(id, metadata);
